...
from app.utils.validators import next_numero_affaire
...
def start_wizard(user: User, annee: int) -> Affaire:
    ...
    Le numéro d'affaire est auto-généré pour l'année donnée (modifiable en Q1).

    Args:
        user: Utilisateur initiateur (sera ``cree_par``).
        annee: Année de l'affaire (pour le préfixe BN{annee}-).

    Returns:
        L'affaire nouvellement créée et persistée (avec ``id`` connu).
    """
    numero = next_numero_affaire(db.session, annee)  # type: ignore[arg-type]
    affaire = Affaire(
...
def save_step(
    ...
    Le routage des champs (colonne typée vs JSONB) est fait selon l'étape :
        - Q1-Q3 : colonnes typées sur ``Affaire``.
    ...
    """
    if step in (StatutWizard.Q1, StatutWizard.Q2, StatutWizard.Q3):
...
def _apply_affaire_columns(affaire: Affaire, payload: dict[str, Any]) -> None:
    """Applique les champs Q1-Q3 sur les colonnes typées de ``Affaire``."""
    column_fields = {
        "annee",
        "numero_affaire",
        "references_internes",
