...
class DimService(TableFormulaireService):
    ...
    HEADER_SECTIONS = [
        SectionSpec(_l("Identification"), [
            FieldSpec("ref_plan", _l("Référence du plan"), "text",
                      required=True, maxlength=100, col_class="col-sm-6 col-md-4"),
            FieldSpec("rev_plan", _l("Révision du plan"), "text",
                      maxlength=20, col_class="col-sm-6 col-md-2"),
            FieldSpec("date_dim", _l("Date du contrôle"), "date",
                      required=True, col_class="col-sm-6 col-md-3"),
            FieldSpec("instrument", _l("Instrument de mesure"), "text",
    ...
    TABLE_SPEC = TableSpec(
        title=_l("Cotes à contrôler"),
        cols=[
            ColSpec("num_cote", _l("N° / Repère"), "text",
                    required=True, maxlength=20, width="w-8"),
            ColSpec("description", _l("Description"), "text",
                    required=True, maxlength=150, width="w-20"),
            ColSpec("valeur_nominale", _l("Nominale (mm)"), "float",
                    required=True, step="0.001", width="w-10"),
            ColSpec("tolerance_plus", _l("Tol. + (mm)"), "float",
                    required=True, step="0.001", min_val="0", width="w-8"),
            ColSpec("tolerance_moins", _l("Tol. - (mm)"), "float",
                    required=True, step="0.001", min_val="0", width="w-8"),
            ColSpec("valeur_mesuree", _l("Mesurée (mm)"), "float",
                    required=True, step="0.001", width="w-10"),
            ColSpec("ecart", _l("Écart (mm)"), "float",
